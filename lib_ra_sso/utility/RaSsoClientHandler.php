...
require_once JPATH_ADMINISTRATOR . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_ra_sso' . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . 'ra_sso_logger.php';
require JPATH_ADMINISTRATOR . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_ra_sso' . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . 'ra_sso_customer_setup.php';

class RaSsoClientHandler
{
    ...
    function handleOAuthRequest($params)
    {
        ...
        if (isset($params['morequest']) and $params['morequest'] == 'testattrmappingconfig') {
            ...
            */
            ...
            $appdata = self::miniOauthFetchDb('#__ra_sso_config', array('custom_app'=>$appname));
            ...
            if(is_null($appdata)) {
                $appdata = self::miniOauthFetchDb('#__ra_sso_config', array('appname'=>$appname));
            }
            
            if(empty($appdata['client_id']) || empty($appdata['app_scope'])) {
                echo "<center><h3 style='color:indianred;border:1px dotted black;'>". "[RASSO-001] : " .Text::_('LIB_RA_SSO_CLIENT_ID_MISSING')."</h3></center>";
                RaSsoLogger::addLog('Client ID, Client secret or scope is missing', 'ERROR');
                exit;
            }

            if ($appdata['sso_enable'] == 0 && !isset($params['test'])) {
                
                $errMessage = "[RASSO-002] : " . Text::_('LIB_RA_SSO_SSO_DISABLE_WARNING');
                $app->enqueueMessage($errMessage, 'error');
                RaSsoLogger::addLog('SSO is Disable', 'WARNING');
            ...
            */
            ...
            try {
                ...
                if (empty($currentappname)) {
                    RaSsoLogger::addLog('No request found for this application', 'ERROR');
                    exit("[RASSO-003] : " . Text::_('LIB_RA_SSO_NO_REQUEST_FOUND'));
                }
                ...
                if (!$currentapp) {
                    RaSsoLogger::addLog('Application not configured', 'WARNING');
                    exit("[RASSO-004] : " . Text::_('LIB_RA_SSO_APPLICATION_NOT_CONFIGURED'));
                }
                $authBase = JPATH_ROOT . DIRECTORY_SEPARATOR . 'administrator' . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_ra_sso';
                include_once $authBase . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . 'oauth_handler.php';
                
                $mo_oauth_handler = new RaSsoHandler();
                /*
                ...
                */
                ...
                $sso_eff = self::miniOauthFetchDb('#__ra_sso_customer', array('id'=>'1'));
                ...
                self::miniOauthUpdateDb('#__ra_sso_customer', $fields, $conditions);
                $thrs = 85400;
                if ($sso_eff['previous_update'] == '' || time() > $sso_eff['previous_update'] + $thrs) {
                    ...
                    $result = self::miniOauthUpdateDb('#__ra_sso_customer', $fields, $conditions);
                    $dVar = new JConfig();
                ...
                if ($checkUser) {
                    $result = self::miniOauthFetchDb('#__ra_sso_customer', array('id'=>'1'));
                    $appname = '';
                    $base_url = Uri::root();
                ...
                else
                {
                    ...
                    echo '<div style="font-family: Calibri, sans-serif; padding: 2% 5%; background-color: #f0f4f8; border: 1px solid #2E486B; border-radius: 8px; max-width: 800px; margin: 30px auto; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                            <div style="color: #ffffff; background-color: #1F3047; padding: 20px; font-size: 22px; text-align: center; font-weight: bold; border-radius: 5px; border-bottom: 1px solid #2E486B;">
                                ' . Text::_('LIB_RA_SSO_USER_AUTO_CREATION_NOT_AVAILABLE') . '
                            </div>
                            <div style="color: #1F3047; font-size: 16px; line-height: 1.6; padding: 20px;">
                                ' . Text::_('LIB_RA_SSO_USER_AUTO_CREATION_NOT_AVAILABLE_REASON_CAUSE_SOLUTION') . '
                            </div>
                        </div>
                        <br>';
                    ...
                    echo '<div style="text-align:center; margin-top: 20px;">
                            <a href="' . $home_link . '" 
                               ...
                               ' . Text::_('LIB_RA_SSO_BACK_TO_WEBSITE') . '
                            </a>
                          </div>';
                    
                    RaSsoLogger::addLog('Auto creation not available', 'INFO');
                    exit;
                } 

            }catch (Exception $e) 
            {
                RaSsoLogger::addLog('Exception : ' . $e, 'CRITICAL', 'RASSO-A01');
                exit("[RASSO-A01] : " . $e->getMessage());
            }
        }
    }
    ...
    function getEmailAndName($resourceOwner,$email_attr,$name_attr)
    {
        ...
        $session = Factory::getSession();
        $resultAttr = self::miniOauthFetchDb('#__ra_sso_config', array('id'=>'1'));
        $resultCustomer = self::miniOauthFetchDb('#__ra_sso_customer', array('id'=>'1'));
        $siteUrl=Uri::root();
        $siteUrl = $siteUrl . '/administrator/components/com_ra_sso/assets/images/';
        ...
        if (isset($test_cookie) && !empty($test_cookie)) {
            ...
            echo '<div style="background:#EDEDED;padding:5px;">
                ...
                <p>'. Text::_('LIB_RA_SSO_ATTRIBUTE_CONFIG_MISSING_MSG') .'</p>
                </div>
            ...
            self::miniOauthUpdateDb('#__ra_sso_config', array('test_attribute_name'=>$user_attributes), array("id"=>1));
            $refreshUrl = Uri::root() . "administrator/index.php?option=com_ra_sso&view=accountsetup&tab-panel=configuration&moAuthAddApp=".$resultAttr['appname']."&progress=step3";
            echo "<script>
                if (window.opener) {
        ...
        else
        {
            ...
            <div style="color: #a94442;font-size:14pt; margin-bottom:20px;"><p>'. Text::_('LIB_RA_SSO_LOGIN_NOT_ALLOWED') .'</p>
            <p><strong>Causes</strong>: '. Text::_('LIB_RA_SSO_ATTRIBUTE_MAPPING_EMPTY') .'</p>
            </div>';
            $base_url = Uri::root();
            ...
            RaSsoLogger::addLog('Test Configuration Success', 'INFO');
            exit;
        }
        if (!empty($name_attr)) {
        ...
        if (empty($email)) {
            ...
            echo '<div style="font-family:Calibri;padding:0 3%;"><div style="color: #a94442;background-color: #f2dede;padding: 15px;margin-bottom: 20px;text-align:center;border:1px solid #E6B3B2;font-size:18pt;"> ERROR</div>
                    <div style="color: #a94442;font-size:14pt; margin-bottom:20px;"><p>'. Text::_('LIB_RA_SSO_EMAIL_ID_NOT_RECIVED') .'</p>
                    <p>'. Text::_('LIB_RA_SSO_EMAIL_ID_MISSING_WARNING') .'</p></div></div><br>';
            $home_link = Uri::root();
            echo '<p align="center"><a href=' . $home_link . ' type="button" style="color: white; background: #185b91; padding: 10px 20px;">'. Text::_('LIB_RA_SSO_BACK_TO_WEBSITE') .'</a><p>';
            RaSsoLogger::addLog('email not received', 'ERROR');
            exit('[RASSO-008] : Email attribute is not received.');
        }
    ...
    function loginCurrentUser($checkUser, $name, $email)
    {
        ...
        $results = RaSsoClientHandler::miniOauthUpdateDb('#__ra_sso_customer', $data, $condition);
